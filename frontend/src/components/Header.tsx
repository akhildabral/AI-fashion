...
export function Header() {
  ...
  const initials = (user?.handle ?? user?.email ?? "?")
    .slice(0, 2)
    .toUpperCase();
      ...
      <div className="relative mx-auto flex h-[76px] max-w-6xl items-center justify-between px-6">
        <Link to="/" className="group flex items-baseline gap-2">
          <span className="font-display text-xl font-extrabold tracking-tight text-ink">
            AI&nbsp;Fashion
            <span className="text-iris transition-colors group-hover:text-iris-deep">
              *
            </span>
          </span>
            ...
            <div className="relative flex items-center gap-2" ref={menuRef}>
              ...
              {menuOpen && (
                <div className="absolute right-0 mt-2 w-52 overflow-hidden rounded-[3px] border border-brass/30 bg-surface py-2 shadow-float">
                  <p className="truncate px-4 py-2 text-xs text-ink/45">
                    {user.email}
                  </p>
                  {MENU.map((item) => (
                    <Link
