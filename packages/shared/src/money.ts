...
let localeHints: string[] = []

/**
 * Locales to guess the currency from, for platforms without a browser
 * `navigator` (the phone passes the device locales from expo-localization).
 */
export function setLocaleHints(langs: string[]) {
  localeHints = langs.filter(Boolean)
}

function deviceLocales(): string[] {
  if (localeHints.length) return localeHints
  if (typeof navigator === 'undefined') return []
  return [navigator.language, ...(navigator.languages ?? [])].filter(Boolean)
}

/** The device's region, as a currency; dirhams when it doesn't say. */
