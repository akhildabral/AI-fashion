...
export function uploadPhoto(image: PickedImage): Promise<PhotoUploadResponse> {
  return apiUpload<PhotoUploadResponse>('/photo', buildImageFormData('photo', image))
}
...
/** POST /api/looks/:id/tryon — render the look onto the user's photo (slow, ~30-40s). */
